...
public class ManitoService {
     ...
     */
    @Transactional
    public Long initiateManito(Long manitoId){
        // 가입된 마니또 회원 목록 조회
        ...
        findManito.updateManito(1, findManito.getEndDateTime());
        return manitoId;
    }
    ...
    public void endManito(Long manitoId) {
        Manito findManito = manitoRepository.findById(manitoId).get();
        findManito.updateManito(2, findManito.getEndDateTime());
    }

    /**
     * 마니또 삭제 매서드
     * 마니또 회원 또한 Cascade on delete로 자동 삭제된다.
     *
     * @param manitoId
     */
    @Transactional
    ...
    @Transactional
    public ManitoDto findManito(Long manitoId) {
        Manito findManito = manitoRepository.findById(manitoId).get();
        return new ManitoDto(findManito.getId(), findManito.getName(), findManito.getEndDateTime(), findManito.getStatus(),
                findManito.getInviteLink(), findManito.getManitoMembers());
    }
    ...
    private HashMap<ManitoMember, ManitoMember> matchManito(List<ManitoMember> manitoMembersInSameGroup) {
        ...
        if (cnt <= 1) throw new IllegalStateException(MANITO_MEMBER_SIZE_AT_LEAST_TWO);
